...
pub fn calculate(chart: &RoxChart, context: &Osu2018DifficultyContext) -> crate::calculator::CalculatorResult<Osu2018Difficulty> {
    let clock_rate = context.clock_rate.unwrap_or(100) as f32 / 100.0;
    
    // Extract OD/KeyCount
    let od_input = context.overall_difficulty.unwrap_or(5.0);
    let column_count = chart.key_count() as usize;
    
    // 1. Hit Window Calculation
    let safe_od = (10.0 - od_input).max(0.0).min(10.0);
    ...
    for (i, &note_idx) in sorted_indices.iter().enumerate() {
        ...
        let current_time = note.time_us as f32 / 1_000.0 / clock_rate; // ms
        ...
        let column_strain_time = if let Some(prev_idx) = last_object_per_column[column] {
            let prev_time = chart.notes[prev_idx].time_us as f32 / 1_000.0 / clock_rate;
    ...
    Ok(Osu2018Difficulty { 
        ...
        object_count: chart.notes.len() as u32,
    })
}
...
mod tests {
    ...
    fn test_osu2018_difficulty_calculation(chart: RoxChart){
        ...
        assert_abs_diff_eq!(result.stars, 6.537083, epsilon = 0.001);
    }
