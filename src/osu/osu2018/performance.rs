...
pub fn calculate(_chart: &RoxChart, diff: &Osu2018Difficulty, context: &Osu2018PerformanceContext) -> crate::calculator::CalculatorResult<Osu2018Performance> {
    ...
    let scaled_score = raw_total_score * (1.0 / score_multiplier);
...
fn compute_strain_value(stars: f32, total_hits: u32, scaled_score: f32) -> f32 {
    ...
    strain_value *= 1.0 + 0.1 * (total_hits as f32 / 1500.0).min(1.0);
...
fn compute_accuracy_value(great_hit_window: f32, strain_value: f32, scaled_score: f32) -> f32 {
    ...
    let accuracy_value = (0.2 - (great_hit_window - 34.0) * 0.006667).max(0.0)
                         * strain_value
...
mod tests {
    ...
    use crate::osu::osu2018::{Osu2018DifficultyContext, difficulty::calculate as osu2018_calculate};
    ...
    fn test_osu2018_performance_calculation(chart: RoxChart){
        ...
        let context = Osu2018PerformanceContext {
            accuracy: 1.0
        };
        let result = calculate(&chart, &diff, &context).expect("Performance calculation failed");

        assert_abs_diff_eq!(result.pp, 511.86728, epsilon = 0.001);
    }
    ...
    fn test_osu2018_performance_calculation_93_71(chart: RoxChart){
        ...
        assert_abs_diff_eq!(result.pp, 428.1258, epsilon = 0.001);
    }
    ...
    fn test_osu2018_performance_calculation_clock_rate(chart: RoxChart){
        ...
        assert_abs_diff_eq!(result.pp, 1407.6055, epsilon = 0.001);
    }
}
